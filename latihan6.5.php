<?php

// Fungsi untuk memformat angka menjadi format mata uang Rupiah
function formatRupiah($angka): string {
    return "Rp " . number_format($angka, 0, ",", ".");
}

// Definisikan Class BelanjaWarung
class BelanjaWarung {
    public $nama;
    public $barang;
    public $harga;
    public $jumlah;
    public $member;

    // Menghitung Subtotal (Harga x Jumlah)
    public function subtotal(): float|int {
        return $this->harga * $this->jumlah;
    }

    // Logika perhitungan diskon berdasarkan status member dan jumlah belanja
    public function diskon($subtotal): int {
        $diskon = 0;

        if ($this->member == true) {
            if ($subtotal > 500000) {
                $diskon = 50000;
            } elseif ($subtotal > 100000) {
                $diskon = 15000;
            }
        } else {
            if ($subtotal > 100000) {
                $diskon = 5000;
            }
        }

        return $diskon;
    }

    // Menghitung Total Akhir (Subtotal - Diskon)
    public function total(): float|int {
        $subtotal = $this->subtotal();
        $diskon = $this->diskon($subtotal);

        return $subtotal - $diskon;
    }
}

// Data simulasi belanja dalam bentuk Array Multidimensi
$dataBelanja = [
    [
        "nama" => "Budi",
        "barang" => "Gula",
        "harga" => 65000,
        "jumlah" => 2,
        "member" => true
    ],
    [
        "nama" => "Sinta",
        "barang" => "Minyak",
        "harga" => 140000,
        "jumlah" => 1,
        "member" => false
    ],
    [
        "nama" => "Rani",
        "barang" => "Tepung",
        "harga" => 50000,
        "jumlah" => 3,
        "member" => true
    ]
];

// Membuat Header Tabel HTML
echo "<table border='1' cellpadding='6'>";
echo "<tr>
        <th>No</th>
        <th>Nama</th>
        <th>Member</th>
        <th>Barang</th>
        <th>Subtotal</th>
        <th>Diskon</th>
        <th>Total</th>
      </tr>";

$no = 1;

// Melakukan perulangan data menggunakan foreach
foreach ($dataBelanja as $d) {
    // Membuat objek baru dari class BelanjaWarung
    $belanja = new BelanjaWarung();

    // Memasukkan data dari array ke dalam properti objek
    $belanja->nama = $d["nama"];
    $belanja->barang = $d["barang"];
    $belanja->harga = $d["harga"];
    $belanja->jumlah = $d["jumlah"];
    $belanja->member = $d["member"];

    // Mengambil hasil perhitungan dari method class
    $subtotal = $belanja->subtotal();
    $diskon = $belanja->diskon($subtotal);
    $total = $belanja->total();

    // Menampilkan baris data ke dalam tabel
    echo "<tr>";
    echo "<td>" . $no . "</td>";
    echo "<td>" . $belanja->nama . "</td>";
    echo "<td>" . ($belanja->member ? "Ya" : "Tidak") . "</td>";
    echo "<td>" . $belanja->barang . "</td>";
    echo "<td>" . formatRupiah($subtotal) . "</td>";
    echo "<td>" . formatRupiah($diskon) . "</td>";
    echo "<td>" . formatRupiah($total) . "</td>";
    echo "</tr>";

    $no++;
}

echo "</table>";

?>